...
>({
  ...
  useEffect(() => {
    ...
    // Update unclustered layer color
    if (map.getLayer(unclusteredLayerId) && prev.pointColor !== pointColor) {
      map.setPaintProperty(unclusteredLayerId, "circle-color", pointColor);
    }

    stylePropsRef.current = { clusterColors, clusterThresholds, pointColor };
  }, [isLoaded, map, clusterColors, clusterThresholds, pointColor, clusterLayerId, unclusteredLayerId]);
  ...
  useEffect(() => {
    ...
    const handleClusterClick = (e: MapLibreGL.MapLayerMouseEvent) => {
      ...
      source.getClusterExpansionZoom(clusterId).then((zoom) => {
        map.easeTo({
          center: coords,
          zoom: Math.min(zoom, (map.getMaxZoom() ?? 18) - 1),
          duration: 500,
        });
      });
    };
